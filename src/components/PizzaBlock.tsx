...
const PizzaBlock: React.FC<PizzaBlockProps> = ({ id, imageUrl, title, types, sizes, price, rating }) => {
  const dispatch = useDispatch();
  const cartItem = useSelector((state: any) => state.cart.items.find((obj: any) => obj.id === id));
  // const cartItem = useSelector(selectItems(id)) - аналогичный вариант селектора выше

  // const [pizzaCount, setPizzaCount] = useState(0);
  const [activeType, setActiveType] = useState(0);
  const [activeSize, setActiveSize] = useState(0);

  // проверяем есть ли у нас этот параметр вообще
